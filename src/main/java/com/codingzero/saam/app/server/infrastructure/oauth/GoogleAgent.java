...
//    private static final String ENDPOINT_GOOGLE_PLUS_GET_PEOPLE = "https://www.googleapis.com/plus/v1/people/me";
    ...
    public String getAuthorizationUrl(Map<String, Object> configurations,
        ...
        OAuth20Service service = oAuth20ServiceFactory.generate(OAuthPlatform.GOOGLE, configurations);
    ...
    public OAuthAccessToken requestAccessToken(
        ...
        try {
            ...
            if (null == token){
                ...
                return new OAuthAccessToken(OAuthPlatform.GOOGLE, userId, accessToken.getAccessToken(), new Date(), expirationTime);
            } else {
                String userId = readGoogleUserId(token);
                long timeout = System.currentTimeMillis() + TIME_OUT;
                Date expirationTime = new Date(timeout);
                return new OAuthAccessToken(OAuthPlatform.GOOGLE, userId, token, new Date(), expirationTime);
            }
        } catch (Exception e) {
