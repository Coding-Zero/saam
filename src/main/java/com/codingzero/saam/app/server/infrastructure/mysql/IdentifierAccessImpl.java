...
public class IdentifierAccessImpl extends AbstractAccess implements IdentifierAccess {
    ...
    public static final String TABLE = "saam_v2.identifiers";
    ...
    public int countByType(String applicationId, IdentifierType type) {
        ...
        try {
            String sql = String.format(
                    "SELECT COUNT(*) FROM %s WHERE "
                            + "application_id=? AND identifier_type=? LIMIT 1;",
                    TABLE);
            stmt = conn.prepareCall(sql);
            stmt.setBytes(1, Key.fromHexString(applicationId).getKey());
    ...
    public List<IdentifierOS> _selectByType(ResultFetchRequest request) {
    ...
    public List<IdentifierOS> _selectByApplicationId(ResultFetchRequest request) {
        String applicationId = (String) request.getArguments()[0];
        Connection conn = getConnection();
        PreparedStatement stmt = null;
        ResultSet rs = null;
