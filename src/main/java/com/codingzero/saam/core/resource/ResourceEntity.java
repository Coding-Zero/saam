...
public class ResourceEntity extends EntityObject<ResourceOS> implements Resource {
    ...
    public Principal getOwner() {
        if (null == owner) {
            owner = principalRepository.findById(getApplication(), getObjectSegment().getPrincipalId());
        }
        return owner;
    }
    ...
    @Override
    public Permission assignPermission(Principal principal, List<Action> actions) {
    ...
    @Override
    public PermissionType verifyPermission(Principal principal, String actionCode) {
        if (principal.getType() == PrincipalType.API_KEY) {
